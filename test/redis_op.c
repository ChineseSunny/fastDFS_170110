...
int rop_set_string(redisContext *conn, char* key, char* value)
{
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]set: %s,value: %s Error:%sn", key, value,conn->errstr);
		retn = -1;
		goto END;
	}
...
int rop_get_string(redisContext *conn, char* key, char* value)
{
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]set: %s,Error:%sn", key,conn->errstr);
		retn = -1;
		goto END;
	}
